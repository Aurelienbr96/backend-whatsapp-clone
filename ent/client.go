...
// QueryContactUser queries the contact_user edge of a Contact.
func (c *ContactClient) QueryContactUser(co *Contact) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := co.ID
		step := sqlgraph.NewStep(
...
func (c *UserClient) QueryContact(u *User) *ContactQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(contact.Table, contact.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, true, user.ContactTable, user.ContactColumn),
		)
		fromV = sqlgraph.Neighbors(u.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		Contact, User []ent.Hook
	}
	inters struct {
		Contact, User []ent.Interceptor
	}
)
