...
// QueryContactUser chains the current query on the "contact_user" edge.
func (cq *ContactQuery) QueryContactUser() *UserQuery {
	query := (&UserClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(contact.Table, contact.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, contact.ContactUserTable, contact.ContactUserColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *ContactQuery) Clone() *ContactQuery {
	...
	return &ContactQuery{
		...
		withContactUser: cq.withContactUser.Clone(),
...
// WithContactUser tells the query-builder to eager-load the nodes that are connected to
// the "contact_user" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ContactQuery) WithContactUser(opts ...func(*UserQuery)) *ContactQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withContactUser = query
	return cq
}
...
func (cq *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if query := cq.withContactUser; query != nil {
		if err := cq.loadContactUser(ctx, query, nodes, nil,
			func(n *Contact, e *User) { n.Edges.ContactUser = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ContactQuery) loadContactUser(ctx context.Context, query *UserQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Contact)
	for i := range nodes {
		fk := nodes[i].ContactUserID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "contact_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *ContactQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withContactUser != nil {
			_spec.Node.AddColumnOnce(contact.FieldContactUserID)
		}
	}
